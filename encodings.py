...
class FreeNeRFEncoding(Encoding):
    """Multi-scale progressively enhanced sinusoidal encodings. Support ``integrated positional encodings`` if covariances are provided.
    ...
    def __init__(
        ...
        if implementation == "tcnn" and not TCNN_EXISTS:
            ...
            encoding_config = self.get_tcnn_encoding_config(max_num_frequencies=self.max_num_frequencies)
            self.tcnn_encoding = tcnn.Encoding(
                n_input_dims=in_dim,
                encoding_config=encoding_config,
            )
    ...
    def get_tcnn_encoding_config(cls, max_num_frequencies , step) -> dict:
        """Get the encoding configuration for tcnn if implemented"""
