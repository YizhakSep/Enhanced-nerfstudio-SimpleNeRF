...
def points_course_loss(median_depth_pos, median_depth_coarse):
    error = median_depth_pos - median_depth_coarse
    loss_map = torch.square(error)
    mse = torch.mean(loss_map)
    return mse, loss_map

def view_course_loss(median_depth_view, median_depth_coarse):
    error = median_depth_view - median_depth_coarse
    loss_map = torch.square(error)
    mse = torch.mean(loss_map)
    return mse, loss_map

def course_fine_consistensy_loss(median_depth_fine, median_depth_coarse):
    error = median_depth_fine - median_depth_coarse
    loss_map = torch.square(error)
    mse = torch.mean(loss_map)
    return mse, loss_map
